...
name<-c("y_log","x1","x3","x4")

newdata<-list()

## recaliberate the data 
...
for (i in 1:rep){
  ...
  lasso[[i]] <- cv.glmnet(newdata[[i]][,2:4], newdata[[i]][,"y_log"],
...
for (i in 1:rep){
  pred[[i]] <- predict(lasso[[i]], s = lasso[[i]]$lambda.min, type="response", 
                       newx = newdata[[i]][,2:4])
  RMSE.pred[i]<-sum(((newdata[[i]][,"y_log"]-pred[[i]])^2))/n
...
for (i in 1:rep){
  ...
  RMSE.pred[i]<-sum(((newdata[[i]][[i]][,"y_log"]-pred[[i]])^2))/n
